...
class Client
{
    ...
    {
        return new PhaxCodes($this);
    }

    public function public()
    {
        return new PhaxioPublic($this);
    }
    ...
    {
        return PhaxCode::init($this)->retrieve($getMetadata);
    }

    public function stringUpload($str, $ext = 'txt') {
        return new StringUpload($str, $ext);
    }
    ...
    public function doRequest($method, $path, $params = array(), $wrapInPhaxioOperationResult = true)
    {
        ...
        switch ($response['status']) {
            case 401:
                throw new Error\AuthenticationException($result['message']);
                break;
            case 404:
                throw new Error\NotFoundException($result['message']);
                break;
            case 422:
                throw new Error\InvalidRequestException($result['message']);
                break;
            case 429:
                throw new Error\RateLimitException($result['message']);
                break;
        }

        if ($response['status'] >= 500 || (isset($result['success']) && $result['success'] != true)) {
            throw new Error\GeneralException($result['message']);
        }

        if ($wrapInPhaxioOperationResult) {
            $opResult = new OperationResult($result['success'], $result['message'], isset($result['data']) ? $result['data'] : null, isset($result['paging']) ? $result['paging'] : null);
        } else {
            $opResult = $response;
        }
    ...
    private function curlRequest($method, $address, $params = array())
    {
        ...
        if ($result === false) {
            throw new Error\APIConnectionException('Curl error: ' . curl_error($handle));
        }
    ...
    private function curlSetoptCustomPostfields($ch, $postfields, $headers = null)
    {
        ...
        foreach ($fields as $field) {
            ...
            if ($value instanceof StringUpload) {
                $body[] = '--' . $boundary;
